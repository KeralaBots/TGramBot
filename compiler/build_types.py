...
def build_types():
    ...
    with open(API_PATH / "api.json") as f:
        ...
        # Main Template
        content_temp = open(TEMPLATES_DESTINATION / "type_class.tmpl",
                            mode="r").read()
        content = ""
        ...
        for name, item in schema.items():
            ...
            class_object = (camel(
                subclass_dict.get(snake(name))) if subclass_dict.get(
            ...
            if fields is None:
                ...
                for field in fields:
                    ...
                    for types in field.get("types"):
                        def_types = (TG_CORE_TYPES.get(types)
                                     if TG_CORE_TYPES.get(types) is not None
                                     else types)
                        if def_types.startswith("Array of Array"):
                            import_type = (TG_CORE_TYPES.get(def_types[18:])
                                           if TG_CORE_TYPES.get(def_types[18:])
                                           is not None else def_types[18:])
                            def_types = (
                                f"List[List[{TG_CORE_TYPES.get(f'{def_types[18:]}')}]]"
                                if TG_CORE_TYPES.get(f"{def_types[18:]}")
                                is not None else
                                f'List[List["{def_types[18:]}"]]')
                        elif def_types.startswith("Array of"):
                            import_type = (TG_CORE_TYPES.get(def_types[9:])
                                           if TG_CORE_TYPES.get(def_types[9:])
                                           is not None else def_types[9:])
                            def_types = (
                                f"List[{TG_CORE_TYPES.get(f'{def_types[9:]}')}]"
                                if TG_CORE_TYPES.get(f"{def_types[9:]}")
                                is not None else f'List["{def_types[9:]}"]')
                        else:
                            import_type = def_types
                            def_types = (def_types if def_types in BASE_TYPES
                                         else f'"{def_types}"')
                        if (import_type not in BASE_TYPES
                                and import_type not in not_pending
                                and import_type not in init_types):
                    ...
                    if name in INPUT_TYPES:
                        if required:
                            ...
                            if field_name == "url":
                                super_class_not_required += (
                                    f'{field_name}: {cust_field} = "", ')
                            else:
                                super_class_not_required += (
                                    f"{field_name}: {cust_field} = None, ")
        ...
        with open(TYPES_DESTINATION_PATH / "tg_types.py", "w+") as type_file:
            with open(TEMPLATES_DESTINATION / "types.tmpl",
                      "r") as type_temp_file:
                type_temp = type_temp_file.read()
                type_file.write(
                    type_temp.format(copyright=COPYRIGHT,
                                     warning=WARNING,
                                     content=content))

        # Constructing __init__.py for types
        with open(TYPES_DESTINATION_PATH / "__init__.py",
                  "w+") as type_init_file:
            with open(TEMPLATES_DESTINATION / "init.tmpl",
                      "r") as type_init_temp_file:
                type_init_temp = type_init_temp_file.read()
                init_imports = ",\n    ".join(init_types)
                # Beautifying the imports
                list_imports = [
                    init_types[i:i + 4] for i in range(0, len(init_types), 4)
                ]
                types_to_import = ""
                for lt in list_imports:
                    types_to_import += (
                        str(lt).replace("[", "").replace("]", "") + ",\n    ")
                type_init_file.write(
                    type_init_temp.format(
                        copyright=COPYRIGHT,
                        warning=WARNING,
