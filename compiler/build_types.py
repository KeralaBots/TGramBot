...
INPUT_TYPES = [
    ...
    "InlineQueryResultArticle",
    "InlineQueryResultAudio" "InlineQueryResultContact",
...
def build_types():
    ...
    with open(API_PATH / "api.json") as f:
        ...
        for name, item in schema.items():
            ...
            class_object = (
                camel(subclass_dict.get(snake(name)))
                if subclass_dict.get(snake(name)) is not None
                else "TelegramObject"
            )
            if fields is None:
                ...
                for field in fields:
                    ...
                    for types in field.get("types"):
                        ...
                        if def_types.startswith("Array of Array"):
                            ...
                            def_types = (
                                def_types
                                if def_types in BASE_TYPES
                                else f'"{def_types}"'
                            )
                        if (
                            import_type not in BASE_TYPES
        ...
        with open(TYPES_DESTINATION_PATH / "tg_types.py", "w+") as type_file:
            with open(TEMPLATES_DESTINATION / "types.tmpl", "r") as type_temp_file:
                ...
                type_file.write(
                    type_temp.format(
                        copyright=COPYRIGHT, warning=WARNING, content=content
                    )
                )
